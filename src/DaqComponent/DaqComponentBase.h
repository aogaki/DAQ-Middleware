...
class DaqComponentBase : public RTC::DataFlowComponentBase
{
 public:
  ...
  virtual ~DaqComponentBase() { delete mytimer; }
 ...
 protected:
  ...
  bool check_header(unsigned char *header, unsigned int received_byte)
  {
    ...
    if (header[0] == HEADER_MAGIC && header[1] == HEADER_MAGIC) {
      ...
      std::cerr << "### ERROR: Bad Magic Num:" << std::hex
                << (unsigned)header[0] << " " << (unsigned)header[1]
                << std::endl;
    }
    std::cerr << std::dec;
    return ret;
  ...
  bool check_footer(unsigned char *footer)
  {
    ...
    if (footer[0] == FOOTER_MAGIC && footer[1] == FOOTER_MAGIC) {
      unsigned int seq_num =
          (footer[4] << 24) + (footer[5] << 16) + (footer[6] << 8) + footer[7];
      if (seq_num == m_loop) {
        ret = true;
      } else {
  ...
  bool check_header_footer(const RTC::TimedOctetSeq &in_data,
                           unsigned int block_byte_size)
  {
    ...
    if (check_header(header, event_byte_size) == false) {
      std::cerr << "### ERROR: header invalid in loop" << m_loop << std::endl;
  ...
  unsigned int get_run_number() { return m_runNumber; }
  ...
  unsigned long long get_sequence_num() { return m_loop; }
  ...
  unsigned long long get_total_byte_size() { return m_totalDataSize; }
  ...
  unsigned long long get_total_event_num() { return m_totalEventNum; }

  bool check_trans_lock() { return m_trans_lock; }

  void set_trans_lock() { m_trans_lock = true; }
  ...
  {
    m_daq_trans_func[CMD_CONFIGURE] =
        &DAQMW::DaqComponentBase::daq_base_configure;
    ...
    m_daq_trans_func[CMD_UNCONFIGURE] =
        &DAQMW::DaqComponentBase::daq_base_unconfigure;
  ...
  BufferStatus check_outPort_status(RTC::OutPort<RTC::TimedOctetSeq> &myOutPort)
  {
    ...
    if (m_debug) {
      std::cerr << "OutPort status: " << RTC::toString(out_status) << std::endl;
    }
    switch (out_status) {
      ...
      case RTC::DataPortStatus::CONNECTION_LOST:
        std::cerr << "OutPort status: " << RTC::toString(out_status)
                  << std::endl;
        ret = BUF_FATAL;
        break;
      ...
      case RTC::DataPortStatus::INVALID_ARGS:
        std::cerr << "Impossible OutPort status: " << RTC::toString(out_status)
                  << std::endl;
        ret = BUF_FATAL;
        break;
    }
  ...
  BufferStatus check_inPort_status(RTC::InPort<RTC::TimedOctetSeq> &myInPort)
  {
    ...
    if (m_debug) {
      std::cerr << "InPort status: " << RTC::toString(in_status) << std::endl;
    }
    switch (in_status) {
      case RTC::DataPortStatus::PORT_OK:
        ret = BUF_SUCCESS;
      ...
      case RTC::DataPortStatus::CONNECTION_LOST:
      case RTC::DataPortStatus::UNKNOWN_ERROR:
        std::cerr << "Impossible InPort status: " << RTC::toString(in_status)
                  << std::endl;
        ret = BUF_FATAL;
        break;
    }
  ...
  int daq_do()
  {
    ...
    if (m_command != CMD_NOP) {       // got other command
      ...
      if (status) {
        ...
        std::cerr << "daq_do: transAction call: illegal command" << std::endl;
      }
      set_done();
    } else {
 ...
 private:
  ...
  int transAction(int command) { return (this->*m_daq_trans_func[command])(); }

  void doAction(int state) { (this->*m_daq_do_func[state])(); }
  ...
  void fatal_report_to_operator(FatalType::Enum type, const char *desc,
                                int code = -1)
  {
